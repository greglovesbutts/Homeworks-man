...
def top_titles
  ...
  top = Movie.select('title', 'id').where('score >= ?', 9.0)
...
def star_wars
  ...
  # hint: use 'select' and 'where'
  stars = Movie.select('id', 'title', 'yr').where('title LIKE \'Star Wars%\'')
end
...
def below_average_years
  ...
  bad_movies = Movie.select('yr', 'COUNT(id) AS bad_movies').where('score < 5').group('yr').order('bad_movies desc')
end
...
def alphabetized_actors
  ...
  alpha_actors = Actor.select('id','name').order('name asc').limit(10)
end
...
def pulp_fiction_actors
  ...
  pulp_jelly_fish = Actor.select('name').join(castings)
end
